...
class TextManager {
	...
	public function toBitmapData(parameters:Dynamic = null):BitmapData {
		...
		var bmd:BitmapData = new BitmapData(Std.int(textRect.width), Std.int(textRect.height), true, Constants.COLOR_TRANSPARENT);
		var textArrayLength:Int = textArray.length;
		var idxGlyph:Int;
		var rect:Rectangle;
		...
		finalBmd.threshold(bmd, bmd.rect, new Point(0,0), "==", Constants.COLOR_BLACK, shadowcolor);	// Convert opaque text to shadow
		finalBmd.threshold(bmd, bmd.rect, new Point(0,0), "==", Constants.COLOR_YELLOW, shadowcolor);	// Convert opaque text to shadow
		
		bmd.threshold(bmd, bmd.rect, new Point(0,0), "==", Constants.COLOR_BLACK, fontColor1);		// Colour opaque text
		bmd.threshold(bmd, bmd.rect, new Point(0,0), "==", Constants.COLOR_YELLOW, fontColor2);		// Colour opaque text
